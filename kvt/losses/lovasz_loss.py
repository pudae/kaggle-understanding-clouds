...
class LovaszSoftmaxLoss(nn.Module):
    ...
    def forward(self, input, target):
        if self.per_image:
            loss = mean(lovasz_softmax_flat(*flatten_probas(prob.unsqueeze(0), lab.unsqueeze(0), self.ignore),
                                            classes=self.classes)
                        for prob, lab in zip(input, target))
        else:
            loss = lovasz_softmax_flat(*flatten_probas(input, target, self.ignore), classes=self.classes)
        return loss
